...
import { ArrowLeft, Plus, Upload, X, ImagePlus, Loader2 } from 'lucide-react';
...
export const PropertyFormPage = () => {
        ...
        <Card>
          ...
          <CardContent>
            <form className="space-y-5" onSubmit={onSubmit}>
              ...
              <Button type="submit" disabled={loading}><Plus className="mr-2 h-4 w-4" />{loading ? t('common.save') : t('property.create')}</Button>
            </form>
          </CardContent>
        </Card>
      </div>
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error('Create property failed', err);
      toast.error('Помилка створення об\'єкта');
    } finally {
      setLoading(false);
    }
  };
        ...
        <Card>
          ...
          <CardContent>
            <form className="space-y-5" onSubmit={onSubmit}>
              ...
              {/* Photo Upload Section */}
              <div className="space-y-3">
                <Label>Фото об'єкта (до 30 зображень)</Label>
                <div
                  className="rounded-lg border-2 border-dashed p-6 text-center cursor-pointer hover:border-primary/50 transition-colors"
                  onClick={() => fileInputRef.current?.click()}
                  onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }}
                ...
                <p className="text-xs text-muted-foreground">
                  Завантажено: {uploadedPhotos.length} / 30
                </p>
              </div>
