...
export const DashboardPage = () => {
  ...
  const cards = [
    { title: role === 'manager' ? 'Мої обʼєкти' : 'Всі обʼєкти', value: stats.properties, icon: Building2 },
    { title: 'Закриті угоди', value: stats.closedDeals, icon: CheckCircle2 },
    { title: 'Конверсія лідів', value: `${stats.conversion}%`, icon: Target },
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2"><Button asChild><Link to="/deals">Воронка угод</Link></Button><Button variant="outline" asChild><Link to="/reports">Звіти</Link></Button></div>
        </div>
        ...
        <Card>
          <CardHeader><CardTitle>Автоматизація</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Підготовлені шаблони email-кампаній і автоматичні follow-up для клієнтів підключаються через інтеграції (Google/SMTP/Webhook). У топ-менеджерів додатково доступні розподіл лідів, призначення задач і контроль документів по угодах.
          </CardContent>
        </Card>
      </div>
    </AppLayout>
