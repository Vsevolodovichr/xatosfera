  ...
  return (
    <AppLayout>
      <div className="space-y-5">
        ...
        <div className="flex gap-2 flex-wrap"><span className="text-sm text-muted-foreground">Drag-and-drop теги:</span>{['терміново', 'ексклюзив', 'без посередників', 'іпотека'].map((tag) => <Badge key={tag} draggable onDragStart={(e) => e.dataTransfer.setData('tag', tag)} className="cursor-grab"><Tag className="mr-1 h-3 w-3" />{tag}</Badge>)}</div>
          ...
          {filtered.map((client) => (
            <Card key={client.id} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const tag = e.dataTransfer.getData('tag'); if (tag) void onTagDrop(client, tag); }}>
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-sm"><span>Сегмент</span><Badge>{segments.find((s) => s.value === client.segment)?.label ?? client.segment}</Badge></div>
                ...
                <div className="flex items-center justify-between text-sm"><span>Історія взаємодій</span><span>{interactions[client.id] ?? 0}</span></div>
